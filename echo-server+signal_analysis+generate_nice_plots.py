...
#%% Sine measurements - new DAQ
filename_cal = 'dataset-Compare-DAQs-good-calibration.hdf5'
t1 = 4.763926391
t2 = 5.04209896571
# 0.3736551105976105 0.4023689031600952 0.34922853112220764 0.3487180173397064 0.3771379590034485 0.33051714301109314 0.0 119.6734882184687 -120.21125079516942

#filename = 'dataset-ruismeting-bad-2.hdf5'
#filename = 'dataset-sine-0.5kHz.hdf5'


#%% Sine measurements - old DAQ
filename_cal = 'data_good__the_calibration_sine_measurements_1V.csv'
t1 = 3.531185
t2 = 3.80935757471
# 0.3720959583297372 0.4026463497430086 0.3483163369819522 0.3481075009331107 0.3771608602255583 0.3300926433876157 0.0 119.67537114598947 -120.20098707610356
#%%
filename = 'data_good_calibration_ruismeting_bad_1V.csv'
filename_spectrum = 'data_good_calibration_ruismeting_bad_1V_spectrum.csv'


#%% Noise performance - new DAQ
filename_cal = 'dataset-calibration-noise-performance.hdf5'
t1 = 7.12151
t2 = 7.6522
# 0.39342746138572693 0.43011873960494995 0.3720490634441376 0.3512530028820038 0.3835946321487427 0.33611997961997986 0.0 119.4834949240884 -120.4838062194755
filename = 'dataset-noise-performance.hdf5'

#%% Noise performance - old DAQ
filename_cal = 'data_good_calibration_noise_performance_1V.csv'
#filename_spectrum = 'data_good_calibration_noise_performance_1V_spectrum.csv'
t1 = 5.7564
t2 = 6.2872
# 0.3915577121078968 0.43040291406214237 0.3703502453863621 0.3506867103278637 0.38361593149602413 0.33574002608656883 0.0 119.48642015849482 -120.50161909606796

filename = 'data_good_calibration_noise_performance_the_measurement_1V.csv'
filename_spectrum = 'data_good_calibration_noise_performance_the_measurement_1V_spectrum.csv'
...
class SignalAnalysis(SignalProc):
    ...
    def __init__(self):
        ...
        self.phi3 = 0
        self.PSDOn = 1
        self.WindowType ='blackmanharris'
        #self.OnePlot = 1
    ...
    def __repr__(self):
        ...
        str3 = "filename of spectrum:".rjust(Rspace)+" %s\n".ljust(0) % self.filename_spectrum
        ...
        #str3 += "Data of csv file:".rjust(Rspace)+" [%d]\n".ljust(0) % size(self.data_csv)
        str3 += "Data spectrum:".rjust(Rspace)+" [%d]\n".ljust(0) % size(self.data_spectrum)
        str3 += "Data spectrum Python:".rjust(Rspace)+" [%d]\n".ljust(0) % size(self.data_spectrum_Python)
    ...
    def readcsvfile(self,filename):
        ...
        try:
            # Change the current working Directory    
            #os.chdir("/home/gebruiker/Km3Net/The_measurements/Data_noise_performance")
            os.chdir("/home/gebruiker/Km3Net/The_measurements/Data_csv_files_and_figs_good_cal")
            print("Directory changed")
        except OSError:
            print("Can't change the Current Working Directory")  
        ...
        self.data_uncal = my_data[:-30000,:]
        #self.data_csv = my_data[:,3:9]
        # Reading in data with: np.savetxt("data.csv", my_data, delimiter=",")
    
    def readcsvfile_spectrum(self,filename_spectrum):
        ...
        from numpy import genfromtxt
        my_data = genfromtxt(self.filename_spectrum, delimiter=',')    
        self.data_spectrum = my_data
    ...
    def Plot_calibrated_data_Matlab(self):
        t = np.linspace(0,(len(self.data_cal)/self.f_s),len(self.data_cal))
        plt.figure(12)
        plt.clf()
        plt.plot(t,self.data_uncal[:,5],label="I\u2080",color = 'darkblue',linewidth=0.2, linestyle='-')
        plt.plot(t,self.data_uncal[:,6],label="I\u2081",color = 'red',linewidth=0.2, linestyle='-')
        plt.plot(t,self.data_uncal[:,7],label="I\u2082",color = 'gold',linewidth=0.2, linestyle='-')
        #plt.title('Amplitude of three Python calibrated signals in time')
        plt.xlabel('Time [s]')
        plt.ylabel('Normalized intensity [V]')
        plt.legend(loc='center left', bbox_to_anchor=(1, 0.5))
        ...
        plt.figure(13)
        plt.clf()
        plt.plot(t,self.data_uncal[:,8],color = 'red',linewidth=0.5, linestyle='-')
        #plt.title('Phase of Python calibrated signals in time')
        plt.xlabel('Time [s]')
        plt.ylabel('Phase [rad]')
        plt.grid()
        
    def Plot_error_of_I_and_Phase(self):
        t = np.linspace(0,(len(self.data_cal)/self.f_s),len(self.data_cal))
        Error_I1 = self.data_uncal[:,5] - self.data_cal[:,0]
        Error_I2 = self.data_uncal[:,6] - self.data_cal[:,1]
        Error_I3 = self.data_uncal[:,7] - self.data_cal[:,2]
        plt.figure(7)
        plt.clf()
        ...
        plt.xlabel('Time [s]')
        plt.ylabel('Absolute error of normalized intensity [V]')
        plt.legend(loc='center left', bbox_to_anchor=(1, 0.5))
        plt.grid()
        Error_Phase_e = abs(self.data_cal[:,3]) - abs(self.data_uncal[:,8])
        plt.figure(8)
        plt.clf()
        plt.plot(t,Error_Phase_e,color = 'darkblue',linewidth=0.2, linestyle='-')
        #plt.title('Error of calibrated phase in time')
        plt.xlabel('Time [s]')
        plt.ylabel('Absolute error of phase [rad]')
        plt.grid()
    ...
    def plotSpect(self,*args):
        ...
        self.Spectrum([0,1,2,3])
        self.data_spectrum_Python = self.PSD[:,0]
        plt.figure(5)
        plt.clf()
        self.plot([0,1,2,3])
        plt.figure(6)
    ...
    def plotSpect_Beethoven(self):
        plt.figure(9)
        data_spectrum = np.zeros((len(self.data_spectrum)-1,4))
        data_spectrum[:,0] = self.data_spectrum[:-1,1]
        data_spectrum[:,1] = self.data_spectrum[:-1,1]
        data_spectrum[:,2] = self.data_spectrum[:-1,1]
        data_spectrum[:,3] = self.data_spectrum[:-1,1]
        self.PSD = data_spectrum
        plt.clf()
        self.plotSpectrumLog([0,1,2,3])
        
    def plotSpect_both(self):
        plt.figure(16)
        data_spectrum = np.zeros((len(self.data_spectrum)-1,4))
        data_spectrum[:,0] = self.data_spectrum[:-1,1]
        data_spectrum[:,1] = self.data_spectrum[:-1,1]
        data_spectrum[:,2] = self.data_spectrum_Python
        data_spectrum[:,3] = self.data_spectrum_Python
        self.PSD = data_spectrum
        plt.legend(loc='center left', bbox_to_anchor=(1, 0.5))
        plt.clf()
        self.plotSpectrumLog([0,1,2,3])
        
    def plotSpect_error(self):
        plt.figure(10)
        data_spectrum = np.zeros((len(self.data_spectrum)-1,4))
        data_spectrum[:,0] = abs(self.data_spectrum[:-1,1] - self.data_spectrum_Python)
        data_spectrum[:,1] = abs(self.data_spectrum[:-1,1] - self.data_spectrum_Python)
        data_spectrum[:,2] = abs(self.data_spectrum[:-1,1] - self.data_spectrum_Python)
        data_spectrum[:,3] = abs(self.data_spectrum[:-1,1] - self.data_spectrum_Python)
        self.PSD = data_spectrum
        plt.clf()
        self.plotSpectrumLog([0,1,2,3])
...
# bij 12.2 - 12.5 sec cal: 0.37036001682281494 0.40459007024765015 0.35400137305259705 0.34533530473709106 0.3795698285102844 0.3335888683795929 0.0 118.86822282454769 -120.24395907780993
# bij 12.2 - 13.2 sec cal: 0.3042308986186981 0.1817348599433899 0.6876006722450256 0.21651670336723328 0.17358481884002686 0.06990432739257812 0.0 174.51425941101218 -152.44478526262375
#%%
filename_cal = 'dataset-Compare-DAQs-good-calibration.hdf5'
t1 = 4.763926391
t2 = 5.04209896571
...
#%%
filename_cal = 'data_the_good_calibration_141_sine_measurements_1V.csv'
t1 = 3.531185
t2 = 3.80935757471
...
#%%
filename = 'dataset-sine-50kHz.hdf5'
# filename = 'data_good_calibration_sine_measurements_40kHz_1V.csv'
filename_spectrum = 'data_good_calibration_sine_measurements_50kHz_1V_spectrum.csv'

#%% Spectrum data for phase of hdf5 file
sig.Data = np.zeros((len(sig.data_cal),4))
...
#%% Spectrum data from old and new DAQ (nice graph for report)
# spectrum_new_DAQ = 10*np.log10(sig.data_spectrum_Python[100:])
# spectrum_old_DAQ = 10*np.log10(sig.data_spectrum[100:-1,1])

spectrum_new_DAQ = sig.data_spectrum_Python[100:]
spectrum_old_DAQ = sig.data_spectrum[100:-1,1]

error = spectrum_old_DAQ - spectrum_new_DAQ
spectrum_error = 10*np.log10(error)

plt.figure(18)
plt.clf()
plt.semilogx(sig.freq[100:],spectrum_error,color = '#d62728ff',linewidth=2)#, linestyle='-')
# plt.semilogx(sig.freq[100:],spectrum_new_DAQ,label="new DAQ",color = '#d62728ff',linewidth=2)#, linestyle='-')
# plt.semilogx(sig.freq[100:],spectrum_old_DAQ,label="old DAQ",color = '#1f77b4ff',linewidth=2)#, linestyle='-')
#plt.title('Amplitude of three Python calibrated signals in time')
plt.xlabel('Frequency [Hz]', fontweight='bold')
plt.ylabel('Error of phase noise [dB re rad/√Hz]', fontweight='bold')
# legend_properties = {'weight':' bold'}
# plt.legend(loc='upper left')#, bbox_to_anchor=(1, 0.5))

# plt.ylabel('Absolute error of phase noise [dB re rad/$\sqrt{Hz}$]')
...
plt.rcParams['figure.titlesize'] = 45

# plt.rcParams['legend.numpoints'] = 1

plt.rcParams['figure.figsize'] = 6.5, 5.5   # figure size in inches
...
#%% Spectrum data from old and new DAQ (nice graph for report)
t = np.linspace(0,(len(sig.data_uncal)/sig.f_s),len(sig.data_uncal))
# I1_new_DAQ = sig.data_uncal[:,0]
30065
t1_old = 5.7564
t2_old = 6.2872
Lng1_old = int(np.round(t1_old*sig.f_s))
Lng2_old = int(np.round(t2_old*sig.f_s))
t1_new= 7.1214
t2_new = 7.6522
Lng1_new = int(np.round(t1_new*sig.f_s))
Lng2_new = int(np.round(t2_new*sig.f_s))

plt.figure(19)
plt.clf()
plt.plot(t[0:int(np.round(0.5308*sig.f_s))],I1_new_DAQ[Lng1_new:Lng2_new]+0.1,label="new DAQ + 0.1 V",color = '#d62728ff',linewidth=4, linestyle='-')
plt.plot(t[0:int(np.round(0.5308*sig.f_s))],I1_old_DAQ[Lng1_old:Lng2_old],label="old DAQ",color = '#1f77b4ff',linewidth=4, linestyle='-')
# plt.plot(t,sig.data_cal[:,2],label="I\u2083",color = '#d62728ff',linewidth=4, linestyle='-')
# plt.plot(t,sig.data_cal[:,3],color = 'red',linewidth=4, linestyle='-')
#plt.title('Amplitude of three Python calibrated signals in time')
plt.xlabel('Time [s]',fontweight='bold')
plt.ylabel('Photodiode output signals [V]',fontweight='bold')
# plt.ylabel('Phase shifted signal [rad]',fontweight='bold')
...
#%%
error = sig.data_uncal[:,8] - sig.data_cal[:,3]

#%% Spectrum data from old and new DAQ (nice graph for report)
t = np.linspace(0,(len(sig.data_uncal)/sig.f_s),len(sig.data_uncal))
plt.figure(20)
plt.clf()
plt.plot(t,error,color = 'red',linewidth=2, linestyle='-')
# plt.plot(t,sig.data_cal[:,3],color = 'red',linewidth=2, linestyle='-')
#plt.title('Amplitude of three Python calibrated signals in time')
plt.xlabel('Time [s]',fontweight='bold')
plt.ylabel('Absolute error of phase shifted signal [rad]',fontweight='bold')
